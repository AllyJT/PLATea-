#!/usr/bin/env bash
# Sweeps RISK_SLOTS / RISK_ADMIT_BUDGET combinations against the real k6
...
#
# Requires main.py to read RISK_SLOTS and RISK_ADMIT_BUDGET from env vars
...
echo "slots,admit_budget,work_score,risk_p95_ms,price_p95_ms,stats_p95_ms,error_rate,qualified" > "$RESULTS_FILE"

for slots in "${SLOTS_VALUES[@]}"; do
  for admit_budget in "${ADMIT_BUDGET_VALUES[@]}"; do
    echo ""
    echo "=== RISK_SLOTS=$slots RISK_ADMIT_BUDGET=$admit_budget ==="

    cleanup
    docker run -d --name "$CONTAINER_NAME" --cpus=2 --memory=2g -p 8080:8080 \
      -e RISK_SLOTS="$slots" -e RISK_ADMIT_BUDGET="$admit_budget" \
      "$IMAGE_NAME" >/dev/null

    for i in $(seq 1 20); do
      curl -sf http://localhost:8080/health >/dev/null && break
      if [ "$i" -eq 20 ]; then
        echo "Health check failed for slots=$slots admit_budget=$admit_budget, skipping."
    ...
    LOG_FILE="$LOG_DIR/slots${slots}_admit${admit_budget}.log"
    ...
    if [ -f sweep-summary.json ]; then
      ...
      echo "$slots,$admit_budget,$WORK_SCORE,$RISK_P95,$PRICE_P95,$STATS_P95,$ERROR_RATE,$QUALIFIED" >> "$RESULTS_FILE"
      ...
      echo "$slots,$admit_budget,ERROR,ERROR,ERROR,ERROR,ERROR,ERROR" >> "$RESULTS_FILE"
    fi
  done
done
