...
echo "slots,queue_timeout,wait_timeout,work_score,risk_p95_ms,price_p95_ms,stats_p95_ms,error_rate,qualified" > "$RESULTS_FILE"

for slots in "${SLOTS_VALUES[@]}"; do
  for timeout in "${TIMEOUT_VALUES[@]}"; do
    for wait_timeout in "${WAIT_TIMEOUT_VALUES[@]}"; do
      echo ""
      echo "=== RISK_SLOTS=$slots RISK_QUEUE_TIMEOUT=$timeout RISK_WAIT_TIMEOUT=$wait_timeout ==="
      ...
      for i in $(seq 1 20); do
        ...
        if [ "$i" -eq 20 ]; then
          echo "Health check failed for slots=$slots timeout=$timeout wait_timeout=$wait_timeout, skipping."
